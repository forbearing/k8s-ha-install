...
function 3_generate_yaml_file {
    ...
    if [[ $(uname) == "Darwin" ]]; then
        ...
        sed="sed"; fi
    rm -rf /tmp/csi-cephfs && cp -r ../csi-cephfs /tmp/
    ...
    for (( i=0; i<${#CEPH_MON_IP[@]}; i++ )); do
        ${sed} -i "s/#CEPH_MON_IP_$i#/${CEPH_MON_IP[i]}/" /tmp/csi-cephfs/3_csi-config-map.yaml; done
    # 2. replace CEPH_CLUSTER_ID
    for YAML_FILE in \
        /tmp/csi-cephfs/3_csi-config-map.yaml \
        /tmp/csi-cephfs/8_storageclass.yaml \
        /tmp/csi-cephfs/9_snapshotclass.yaml; do
        ${sed} -i "s/#CEPH_CLUSTER_ID#/${CEPH_CLUSTER_ID}/" ${YAML_FILE}; done
    # 3. replace CEPH_RBD_POOL
    ${sed} -i "s/#CEPH_FS_POOL#/${CEPH_FS_POOL}/" /tmp/csi-cephfs/8_storageclass.yaml
    # replace CEPH_USER and CEPH_USER_KEY
    ${sed} -i "s/#CEPH_USER#/${CEPH_USER}/" /tmp/csi-cephfs/7_secret.yaml
    ${sed} -i "s/#CEPH_USER_KEY#/${CEPH_USER_KEY}/" /tmp/csi-cephfs/7_secret.yaml
    ...
    for YAML_FILE in /tmp/csi-cephfs/*.yaml; do
        ${sed} -i "s/#CEPH_NAMESPACE#/${CEPH_NAMESPACE}/" ${YAML_FILE}; done
    # 2. replace CEPH_RBD_SC
    ${sed} -i "s/#CEPH_FS_SC#/${CEPH_FS_SC}/" /tmp/csi-cephfs/8_storageclass.yaml
    # 3. replace CEPH_RBD_SC_SNAPSHOT
    ${sed} -i "s/#CEPH_FS_SC_SNAPSHOT#/${CEPH_FS_SC_SNAPSHOT}/" /tmp/csi-cephfs/9_snapshotclass.yaml
}
...
function 4_deploy_ceph_csi {
    ...
    kubectl apply -f /tmp/csi-cephfs/
}
