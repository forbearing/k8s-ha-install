...
function 1_install_docker {
    ...
    while true; do
        if curl -fsSL ${docker_url}/linux/debian/gpg | gpg --yes --dearmor -o /usr/share/keyrings/docker-archive-keyring.gpg; then
            break; fi
        sleep 1
    done
    echo "deb [arch=amd64 signed-by=/usr/share/keyrings/docker-archive-keyring.gpg] ${docker_url}/linux/debian $(lsb_release -cs) stable" | \
